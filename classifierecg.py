...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        if os.path.isfile(args.pretrained):
            ...
            # checkpoint = torch.load(args.pretrained, map_location="gpu")
            checkpoint = torch.load(args.pretrained, map_location=f"cuda:0")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        validate(val_loader, model, criterion, args, epoch)# logger,
...
def validate(val_loader, model, criterion, args, epoch):#logger,
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    progress = ProgressMeter(
        len(val_loader),
